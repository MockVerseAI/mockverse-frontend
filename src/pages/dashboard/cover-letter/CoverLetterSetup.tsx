...
import CoverLetterService, { ICoverLetterCreate } from "@/services/coverLetterService";
...
const CoverLetterSetup = () => {
  ...
  const { mutate: createCoverLetter, isPending } = useMutation({
    mutationFn: (payload: ICoverLetterCreate) => {
      return CoverLetterService.create(payload);
    },
    onError: (e: any) => {
    ...
    onSuccess: (res: any) => {
      const { data } = res;
      navigate(`/dashboard/cover-letter/report/${data?.data?._id}`);
    },
  });
  ...
  const onSubmit = useCallback(
    ...
    [createCoverLetter, selectedResume]
  );
...
export default CoverLetterSetup;
